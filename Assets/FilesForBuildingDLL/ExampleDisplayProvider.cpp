...
UnityXRProjection ExampleDisplayProvider::GetProjection(int pass)
{
    ...
    ret.type = kUnityXRProjectionTypeHalfAngles; //kUnityXRProjectionTypeMatrix
    if (pass == 0) {
  /*      ret.data.matrix.columns[0] = {1,0,0,0};
        ret.data.matrix.columns[1] = {0,1,0,0};
        ret.data.matrix.columns[2] = {0,0,1,0};
        ret.data.matrix.columns[3] = {0,0,1,0};*/
        ret.data.halfAngles.left = -0.55;
        ret.data.halfAngles.right = 0.5;
        ret.data.halfAngles.top = 0.25;
        ret.data.halfAngles.bottom = -0.25;
    }
    else {
        ret.data.halfAngles.left = -0.75;
        ret.data.halfAngles.right = 0.7;
        ret.data.halfAngles.top = 0.55;
        ret.data.halfAngles.bottom = -0.55;
        //ret.data.matrix.columns[0] = { 2,0,0,0 };
        //ret.data.matrix.columns[1] = { 0,2,0,0 };
        //ret.data.matrix.columns[2] = { 0,0,2,0 };
        //ret.data.matrix.columns[3] = { 0,0,2,0 };
    }
    return ret;
}
