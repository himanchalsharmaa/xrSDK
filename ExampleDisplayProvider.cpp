...
private:
#if XR_DX11
    std::vector<std::vector<ID3D11Texture2D*>> m_NativeTextures;
#else
...
UnitySubsystemErrorCode ExampleDisplayProvider::GfxThread_PopulateNextFrameDesc(const UnityXRFrameSetupHints& frameHints, UnityXRNextFrameDesc& nextFrame)
{
    ...
    else
    {
        ...
        cullingPass.separation = 0.0625f;
    }
...
{
    const int texWidth = (int)(1067.0f * requestedTextureScale * (SIDE_BY_SIDE ? 2.0f : 1.0f));
    const int texHeight = (int)(269 * requestedTextureScale);


    m_NativeTextures.resize(numTextures);
    for (int i = 0; i < m_NativeTextures.size(); i++) {
        m_NativeTextures[i].resize(2);
    }
    ...
    for (int i = 0; i < numTextures; ++i)
    {
        ...
        D3D11_TEXTURE2D_DESC dxDesc1 = CD3D11_TEXTURE2D_DESC(DXGI_FORMAT_D32_FLOAT_S8X24_UINT, texWidth, texHeight);
        dxDesc1.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_DEPTH_STENCIL;
        dxDesc1.ArraySize = textureArrayLength == 0 ? 1 : textureArrayLength;;
        ID3D11Texture2D* depthTex;
        dxDevice->CreateTexture2D(&dxDesc1, NULL, &depthTex);

        m_NativeTextures[i][0] = nativeTex;
        m_NativeTextures[i][1] = depthTex;

        uDesc.color.nativePtr = nativeTex;
     //   uDesc.colorFormat = kUnityXRRenderTextureFormatBGRA32;
       // uDesc.depthFormat = kUnityXRDepthTextureFormat16bit;
       // uDesc.flags = kUnityXRRenderTextureFlagsLockedWidthHeight;
        uDesc.depth.nativePtr = depthTex;
#else
        ...
        uDesc.textureArrayLength = textureArrayLength == 0 ? 1 : textureArrayLength;;
       ...
       // uDesc.depth.referenceTextureId = uDesc.color.referenceTextureId = uTexId;
    }
}
...
#if XR_DX11
            m_NativeTextures[i][0]->Release();
            m_NativeTextures[i][1]->Release();
#endif
        }
    }
...
UnityXRProjection ExampleDisplayProvider::GetProjection(int pass)
{
    ...
    //}
    //else {
    //    ret.data.halfAngles.left = -.5;
    //    ret.data.halfAngles.right = .5;
    //    ret.data.halfAngles.top = 0.625;
    //    ret.data.halfAngles.bottom = -0.625;
    //}
    ...
    UnityXRProjection ret;
    if (pass != 2) {
        ret.type = kUnityXRProjectionTypeHalfAngles; //kUnityXRProjectionTypeMatrix //kUnityXRProjectionTypeHalfAngles
    }
    else {
        ret.type = kUnityXRProjectionTypeMatrix;
    }
    if (pass == 0) {
        //ret.data.matrix.columns[0] = {2.515768,0,0,0};
        //ret.data.matrix.columns[1] = {0,3.975894,0,0};
        //ret.data.matrix.columns[2] = {0.04978702,- 0.01305255,-1.004008,-1};
        //ret.data.matrix.columns[3] = {0,0,-0.2004008,0};
        // 
        //ret.data.halfAngles.left = -1.0;
        //ret.data.halfAngles.right = 1.0;
        //ret.data.halfAngles.top = 0.625;
        //ret.data.halfAngles.bottom = -0.625;
        ret.data.halfAngles.left = -0.37770294;
        ret.data.halfAngles.right = 0.417282292;
        ret.data.halfAngles.top = -0.25479868;
        ret.data.halfAngles.bottom = 0.24823283;
    }
    else if (pass == 1) {
        ret.data.halfAngles.left = -0.37814267;
        ret.data.halfAngles.right = 0.41765297;
        ret.data.halfAngles.top = -0.24738527;
        ret.data.halfAngles.bottom = 0.25719569;
        //ret.data.matrix.columns[0] = { 2.513208,0,0,0 };
        //ret.data.matrix.columns[1] = { 0,3.963685,0,0 };
        //ret.data.matrix.columns[2] = { -0.04961488,0.01944271,-1.004008,-1};
        //ret.data.matrix.columns[3] = { 0,0,-0.2004008,0 };
    }
    else if (pass == 2) {
        float vrL = -0.37770294;
        float vrR = 0.41765297;
        float vrB = 0.24823283;
        float vrT = -0.24738527;
        float x, y, a, b, c, d, e;

        float nearval = 0.3f;
        float farval = 350.0001996f;
...
UnitySubsystemErrorCode ExampleDisplayProvider::QueryMirrorViewBlitDesc(const UnityXRMirrorViewBlitInfo* mirrorBlitInfo, UnityXRMirrorViewBlitDesc* blitDescriptor, ProviderContext& ctx)
{
    ...
    const UnityXRVector2 sourceTextureSize = {static_cast<float>(1067), static_cast<float>(269)};
